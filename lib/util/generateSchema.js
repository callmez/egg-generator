...
module.exports = async auto => { // see sequelize-auto/lib/index => run()
  return new Promise((resolve, reject) => {
    var self = auto;
    var text = {};
    var tables = [];
    var typescriptFiles = [ '', '' ]; // no typescript

    const schema = {};
    var callback = () => resolve(schema);
    auto.build(generateText);
    function generateText(err) {
      ...
      async.each(_.keys(self.tables), function(table, _callback){
        ...
        // text[table] += ");\n};\n";
        schema[table] = {
          fields: self.tables[table], // TODO get column comment info
          text: text[table],
        };
        _callback(null);
      }, function(){
